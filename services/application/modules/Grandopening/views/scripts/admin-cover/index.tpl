...
<script type="text/javascript">
    ...
    var absBasePath = '<?php echo 'http://' . $_SERVER['HTTP_HOST'] . $this->baseUrl() . '/public/opening_cover/'; ?>';
...
<?php endif; ?>
<div>
    <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Upload New Covers'), array('id' => 'demo-browse', 'class' => 'buttonlink admin_files_upload', 'onclick' => 'showFallbackUploader();')) ?>
</div>
    ...
    <div class="admin_files_wrapper">
        ...
        <form action="<?php echo $this->url(array('action' => 'delete')) ?>?path=<?php echo $this->relPath ?>" method="post">
            <ul class="admin_files">
                <?php
                foreach ($this->paginator as $content): $i++;
                    $id = 'admin_file_' . $i;
                    $contentKey = $content['rel'];
                    ?>
        ...
        <?php echo $this->formCheckbox('actions[]', $content['rel']) ?>
                        ...
                        <div class="admin_file_options">
                            ...
                            | <a href="<?php echo $this->baseUrl() . '/pages/grandopening?bg=' . $contentKey ?>" target="_blank"><?php echo $this->translate('preview') ?></a>
                            <?php if ($this->cover != $contentKey) : ?>
                                | <?php echo $this->htmlLink($this->url(array('action' => 'cover')) . '?bg=' . $contentKey, $this->translate('set as cover')) ?>
                            <?php endif ?>
                        </div>
            ...
            <?php echo $content['name'] ?>
                        ...
                        <div class="admin_file_preview admin_file_preview_<?php echo $content['type'] ?>" style="display:none">
                            <?php if ($content['is_image']): ?>
                                <?php echo $this->htmlImage($this->baseUrl() . '/public/opening_cover/' . $content['rel'], $content['name']) ?>
