...
class User_AuthController extends Core_Controller_Action_Standard
{
  public function loginAction()
  {
	...
	$this->view->facebook = $settings->getSetting( 'user.signup.facebook', 1 );
...
//    }
    ...
    if( null === $this->_helper->contextSwitch->getCurrentContext() ) {
      ...
      // check if has data on activity-feed if not redirect to explore
      $select = Engine_Api::_()->getDbtable('stream', 'whmedia')->selectStreamProjects($user);
      $activityFeed = Zend_Paginator::factory($select);

      if( $activityFeed->getTotalItemCount() == 0 ) {
        return $this->_helper->redirector->gotoRoute(array( 'controller' => 'search', 'type' => 'tags' ), 'default', true); 
      }else
        // Just redirect to home
        return $this->_helper->redirector->gotoRoute(array('action' => 'activity-feed'), 'mediamasonry', true);
    }
  }
