...
class User_SettingsController extends Core_Controller_Action_User
{
  ...
  public function profileAction(){
	...
	//$emailNotifTable = Engine_Api::_()->getDbTable( 'emailNotifications', 'activity' );
	//$emailNotifTable->check( $viewer );
  } 
  ...
  public function passwordAction()
  {
	...
	$this->view->facebookSigned = 0;
	// if current user is signed using facebook
	// then remove the old password field
	if ( $this->getRequest()->getParam( 'type' ) == 1 ) {
		$this->view->facebookSigned = 1;
  ...
  public function emailsAction()
  {
		...
		$modules = Engine_Api::_()->getDbtable('modules', 'core')->getModulesAssoc();
		$notificationTypes = Engine_Api::_()->getDbtable('notificationTypes', 'activity')->getNotificationTypes();
		$notificationSettings = Engine_Api::_()->getDbtable('emailNotifications', 'activity')->getEnabledNotifications($user);
		...
		// Make form
		$this->view->form = $form = new Engine_Form(array(
		  'title' => 'Emails Notification Settings',
		  'description' => 'Which of the these do you want to receive email alerts about?',
		));
		...
		try {
		// Set notification setting
		Engine_Api::_()->getDbtable('emailNotifications', 'activity')
			->setEnabledNotifications($user, $values);
		} catch( Exception $e ) {
			print_r( $e ); exit;
		}
