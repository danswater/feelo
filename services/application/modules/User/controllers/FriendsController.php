...
class User_FriendsController extends Core_Controller_Action_User
{
  ...
  public function confirmAction()
  {
    ...
    try {
/*
      if (!$followApi->isFollow($viewer, $user)) {
        $followApi->Follow($viewer, $user);
        $notifier = Engine_Api::_()->getDbtable('notifications', 'activity')
          ->addNotification($user, $viewer, $user, 'friend_follow_accepted');

        $notifier = Engine_Api::_()->getDbtable('notifications', 'activity')
          ->addNotification($viewer, $user, $viewer, 'friend_follow');  
      }
*/
      $row = $followApi->fetchRow( array(
    ...
    $row->save();
      ...
      $message2 = $user->displayname . " is now following you." ;
  ...
  public function ignoreAction()
  {
    ...
    try {
      if ($followApi->isFollow($viewer, $user)) {
        $followApi->unFollow($viewer, $user);
      }
      ...
      $message = sprintf($message, $user->__toString());
      $message2 = "You ignored " . $user->displayname . "s request to follow you";
