...
function DropDownHover(hover, target){

  var circleEvent = {
    cTimeOut : null,
    mouseenter : function(){
      if(circleEvent.cTimeOut != null)
        clearTimeout(circleEvent.cTimeOut);
      $(target).setStyle("display", "block");     
    },
    mouseleave : function(){
      circleEvent.cTimeOut = setTimeout(function(){
        $(target).setStyle("display", "none");      
        circleEvent.cTimeOut = null;
      }, 300)
    }
  }
  $(hover).addEvents(circleEvent)
  $(target).addEvents(circleEvent)
