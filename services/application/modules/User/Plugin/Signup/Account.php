...
class User_Plugin_Signup_Account extends Core_Plugin_FormSequence_Abstract
{
  ...
  public function onProcess()
  {
    ...
    $emailFragment = explode( '@', $data[ 'email' ] );
    $data[ 'username' ] = $emailFragment[ 0 ];
	
    // lets check if the username we provide
    // is haven't been used.
    // if used lets append and integer
    $userTable = Engine_Api::_()->getDbtable('users', 'user');
    $user = $userTable->fetchRow($userTable->select()->where('username = ?', $data[ 'username' ] ) );

    if( $user->username ) {
	  $identifier = mt_rand();
      $data[ 'username' ] = $data[ 'username' ] . $identifier;
    }
	*/
	
    // lets check the users timezone and convert it 
    // to abbrv timezone
    $timezone = new User_Library_DateTimezone( User_Library_DateTimezone::tzOffsetToName( $data[ 'timezone' ] ) );
    ...
    // lets just set the language to default en
    // to prevent an error while creating user
    ...
    switch( $settings->getSetting('user.signup.verifyemail', 0) ) {
      ...
      case 2:
		...
		$mailParams['verify_code'] = $verify_row->code;
  ...
  public function onFacebookVerified () {
	...
	if ( !isset( $data[ 'email' ] ) ) {
		$data[ 'email' ] = strtolower( $data[ 'first_name' ] ) . strtolower( $data[ 'last_name' ] ) .'@facebook.com';
	}
	$userTable = Engine_Api::_()->getDbtable('users', 'user');
	$user = $userTable->fetchRow($userTable->select()->where('email = ?', $data[ 'email' ] ) );
	...
	$data[ 'displayname' ] = strtolower( $data[ 'first_name' ] ) . strtolower( $data[ 'last_name' ] );	
