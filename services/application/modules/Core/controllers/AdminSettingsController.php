 ...
 * @version    $Id: AdminSettingsController.php 9778 2012-09-11 23:53:52Z matthew $
 * @author     John
 */
...
//        }
  ...
  public function performanceAction()
  {
    ...
    if(  $form->getElement('translate_array')->getValue() == 1 && $this->getRequest()->isPost() ) {
      ...
      // Check that exec() function is availible
      if (!is_callable('exec')) {
        $form->getElement('translate_array')->setValue(0);
        $form->addError('PHP Arrays could not be validated. - PHP exec() function disabled');
        return;
        }
      // Check that php is command line accessible
      else{
        $output = exec('php --version');
        if($output == NULL){
          $form->getElement('translate_array')->setValue(0);
          $form->addError('PHP Arrays could not be validated - Incorrect PHP Path');
          return;
          }
        }
