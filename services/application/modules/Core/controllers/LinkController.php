 ...
 * @version    $Id: LinkController.php 9747 2012-07-26 02:08:08Z john $
 * @author     John
 */
...
//    }
  ...
  protected function _previewHtml($uri, Zend_Http_Response $response,$returnDataAsArray = false)
  {
    $body = $response->getBody();
    $body = trim($body);
    if( preg_match('/charset=([a-zA-Z0-9-_]+)/i', $response->getHeader('content-type'), $matches) ||
...
//    }
    ...
    $this->view->imageCount = count($newimage);
    $this->view->images = $newimage;

    if($returnDataAsArray === true){
      return array(
  ...
  public function scrapeAction(){
    if( $this->getRequest()->isPost() ){
      ...
      try
      {
        ...
        $response = $this->_previewHTML($uri, $response,true);
