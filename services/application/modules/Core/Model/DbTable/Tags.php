...
class Core_Model_DbTable_Tags extends Engine_Db_Table
{
  ...
  /**
   * get featured hashtag who being like by admin
   */

  public function adminLikeHashtag( $filter = array() ) {
    $filter[ "offset" ] = isset( $filter[ "offset" ] ) ? $filter[ "offset" ] : 50;

    return $this->getDefaultAdapter()->query( "SELECT *
      FROM engine4_core_tagmaps AS a
      LEFT JOIN engine4_core_tags AS b ON b.tag_id = a.tag_id
      LEFT JOIN engine4_whmedia_followhashtag AS c ON c.hashtag_id = b.tag_id
      LEFT JOIN engine4_users AS d ON d.user_id = c.follower_id
      WHERE d.level_id IN ( 1, 2 ) ORDER BY RAND() LIMIT " . $filter[ "offset" ] )->fetchAll();
