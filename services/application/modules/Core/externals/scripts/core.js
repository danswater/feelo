  ...
  send : function(req, options){
    ...
    req.addEvent('success', function(response, response2, response3, response4){
      ...
      if(typeof response.status != "undefined" && response.status == false){
		...
		if ( options.yambaCallback ) {
			options.yambaCallback();
		}
        return;  
      }
...
function removeAllPopupOverlay(){
  $("profile-info").setStyle("display", "none");

  $("minitop_update_pulldown").removeClass('updates_pulldown_active');
  $("minitop_update_pulldown").addClass('updates_pulldown')

  $('custom-wbody-overlay').setStyle('display', 'none');
}


window.addEvent('domready', function() {
	if( $( "custom-wbody-overlay" ) != null) 
		$("custom-wbody-overlay").addEvent('click', function(){
