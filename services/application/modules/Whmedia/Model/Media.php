   ...
   public function getThumb($thumbWidth = null, $thumbHeight = null, $crop = true) {
      ...
      if (empty ($file_path)) {
          if ($file_out !== null) {
			...
			$file_out = Engine_Api::_()->getItemTable('storage_file')->fetchAll(array('parent_type = ?' => 'whmedia_media',
																						'parent_id = ?' => $this->media_id,
																						'type = ?' => 'thumb.etalon'))->current();			
			
			if ( $file_out ) {
				$file_path = $file_out->storage_path;
			} else {
				$file_path = 'application/modules/Whmedia/externals/images/no_photo_project.png';
			}
		  }
	  }
...
EOF;
   ...
   /*
        $this->_file = Engine_Api::_()->getItemTable('storage_file')->fetchRow(array('parent_id = ?' => $this->media_id,
                                                                                        'type = ?' => 'video.html5'));
*/
		if ($this->_file === null) {
		...
		if( $this->_file->mime_major === 'video' ) {
			$row = Engine_Api::_()->getItemTable('storage_file')->fetchRow(array('parent_id = ?' => $this->media_id,
                                                                                        'type = ?' => 'video.html5'));
   ...
   protected function videoServices_Embedded() {
       ...
       try {
           if ($data['type'] == 'youtube'){
            ...
            // 
            $emb = '<iframe src="https://www.youtube.com/embed/'.$data['code'].'" width="640" height="360" frameborder="0"></iframe>';
           }
           if ($data['type'] == 'vimeo')
...
$emb = <<<EOF
         <iframe src="https://www.youtube.com/embed/{$code}" width="320" height="240" frameborder="0"></iframe>
EOF;
      return $emb;
  }
...
EOF;
  ...
  public function getMediaTypeLabel(){
    $type = strtolower($this->getMediaType());
  ...
  protected function _urlEmbedded() {
      ...
      if (!empty($file)) {        
        $img_width = (int)Engine_Api::_()->getApi('settings', 'core')->getSetting('image_width', '600');
        $out .= '<a href="'. $this->is_url .'" target="_blank"><img alt="Media" src="'. $file->map() .'" style="max-width: ' . $img_width . 'px;" /></a><br/>';
      }
      if (!empty($this->title)) {
