...
function projects_viewmore(identity, addition_data, isMobile) {
    ...
    if(fun_max_projects_page == 1) return;
...
whmedia.project_follow = new Class({
    ...
    togglefollow: function(user_id, is_req) {
        ...
        if (!a_follower_button.hasClass('unfollow')) {
            ...
            try{
                ...
                if (typeOf(div_boxes) == 'element') {
                    Smoothbox.open(div_boxes, {mode: 'Inline', width: '700', height: '500'});
                    $$(".gotoadnewbox").setProperty('cuid', user_id);
                }
            } catch( e ) { console.log( e ); }

        }
        this.run_JSON('toggle-follow', {'user_id': user_id, 'is_req' : is_req}, function(response) {
            try {
                if (response.isfollow) {
                    var textResponse = en4.core.language.translate("unFollow");
                    if( response.is_req == 1 ){
                        textResponse = "Approval";
                    }
                    $$('a.follower_button_' + user_id).addClass('unfollow').set('text', textResponse);
                }
                else {
