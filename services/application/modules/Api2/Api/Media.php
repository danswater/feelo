...
class Api2_Api_Media extends Core_Api_Abstract {
	...
	*/
	...
	public function fetchPhotoMediaWithImageSizesByProjectId ( $projectId, $coverFileId ) {
		// Fetch Media(s)
		$api2Media = Engine_Api::_()->getDbTable( 'medias', 'api2' );
		$mediaRows = $api2Media->readPhotoMediaByProjectId( $projectId );

		// Lets just get the first array so that we
		// can extract the media information
		$tmp = $mediaRows->toArray();
		$mediaRow = $tmp[ 0 ];
		unset( $tmp );

		// get image paths with different sizes
		$imagePaths = array();
		foreach( $mediaRows as $key => $media ) {
			if ( $media->type != '' ) {
				$imagePaths[ $media->type ] = $media->storage_path;
			}
		}
		
		$imagesizes = getimagesize( $imagePaths[ 'original' ] );
		...
		$media->setProjectId( $mediaRow[ 'project_id' ] );
		$media->setUrl( $mediaRow[ 'url' ] );
		$media->setStoragePath( $imagePaths[ 'original' ] );
		$media->setExtraLarge( $imagePaths[ 'extra_large' ] );
		$media->setLarge( $imagePaths[ 'large' ] );
		$media->setMedium( $imagePaths[ 'medium' ] );
		$media->setSmall( $imagePaths[ 'small' ] );
		$media->setTypee( $mediaRow[ 'type' ] );
		$media->setMediaCode( $mediaRow[ 'code' ] );
		$media->setImageWidth( $imagesizes[ 0 ] );
		$media->setImageHeight( $imagesizes[ 1 ] );
	...
	public function fetchMediaByProjectIdWithStructuredResponse ( $projectId, $coverFileId ) {
		$apiMedia = Engine_Api::_()->getApi( 'whmedia', 'api' );
		$mediaRow = $apiMedia->fetchMediaDetails( $projectId, $coverFileId );
