 ...
 * @version    $Id: Notifications.php 9747 2012-07-26 02:08:08Z john $
 * @author     John
 */
...
class Activity_Model_DbTable_Notifications extends Engine_Db_Table
{
          ...
          Core_Model_Item_Abstract $object, $type, array $params = null)
  {
    ...
    if( $notificationSettingsTable->checkEnabledNotification($user, $type) && !empty($user->email) )
    {
      ...
      // send data for gcm and push notification
      $mobileSender = new Api_Library_MobileNotifier();
      $mobileSender->send( $subject, $type );

      
      $emailnotificationSettingsTable = Engine_Api::_()->getDbtable('emailNotifications', 'activity');
      if( $emailnotificationSettingsTable->checkEnabledNotification($user, $type) && !empty($user->email) ) {
   ...
   */
  public function getNotificationsPaginator(User_Model_User $user, $userType = "")
  {
    $enabledNotificationTypes = array();
    foreach( Engine_Api::_()->getDbtable('NotificationTypes', 'activity')->getNotificationTypes() as $type ) {
      if( $type->type == "friend_follow_request" ) continue;
      $enabledNotificationTypes[] = $type->type;
    }

    if( $userType != "" ) 
      $enabledNotificationTypes = array( $userType );
  ...
  public function getNotificationsUnreadAndMobileSendPaginator(User_Model_User $user, $userType = "")
  {
    $enabledNotificationTypes = array();
    foreach( Engine_Api::_()->getDbtable('NotificationTypes', 'activity')->getNotificationTypes() as $type ) {
      if( $type->type == "friend_follow_request" ) continue;
  ...
  public function hasRequestOrNotification( User_Model_User $user, $isRequest = false ) {
  ...
  public function changeMobileSendStatus( $notificationId, $status ){

    $this->update( array( 'mobile_send' =>  $status, "read" => 1 ), array( "notification_id = ?" => $notificationId ) ); 
