    ...
    add_video_service: function (type, code) {
        ...
        new Request.JSON({
            ...
            'onSuccess' : function(responseObject) {
                ...
                var vs = new Element('div', {
                    id: 'vs_' + responseObject.media_id
                });
                var img_type_src = '';
                if (type == 'youtube')
...
whmedia.cover_media = new Class({
    ...
    initialize:function(options){
        this.parent(options);
        this.loading_image = new Asset.image('application/modules/Core/externals/images/loading.gif',{
            id : 'wh_loader'
        });
    },
    getURL : function(action) {
    ...
    set_cover: function(id) {
        ...
        this.run_JSON('set-cover', {
            'id' : id
        }, function(response) {
            parent.wh_project.updateMediaBlock(this.options.video_id);                                                                      
            new Element('div', {
                'class':'global_form_popup_message',
                'text': parent.en4.core.language.translate("Video cover saved.")
            }).replaces(covers);
...
var wh_leafing = new Class({
    ...
    initialize:function(a){
        ...
        if (wh_leafing_instance == false) {
            wh_leafing_instance = new Keyboard({
                active: true
            });
        }
...
whmedia.edit_layout = new Class({
    ...
    init_sort:function() {
        var bind = this;
        this.sort = new Fx.Sort($$('#media_container .media_div'), {
            mode: 'vertical',
            ...
            onComplete: function(){
                ...
                if (bind.tinyMCE_actived.length > 0) {
                    while (bind.tinyMCE_actived.length > 0) {     
                        tinyMCE.execCommand('mceAddControl', false, bind.tinyMCE_actived.pop());                                                                                
                    }
                }
            }
        });
    ...
    edit_media_title: function (media_id) {
        ...
        new Element('button', {
            type:"button",
            text:en4.core.language.translate("Save")
        }).addEvent('click', function(){
        ...
        new Element('button', {
            type:"button",
            text:en4.core.language.translate("Cancel")
        }).addEvent('click', function(){
            media_div.getElement('.wh_edit_media_controls').setStyle('display', '');
        ...
        Smoothbox.open('', {
            ...
            delete_media: function(smooth) {                                                          
                ...
                function(response) {
                    ...
                    new Element('p', {
                        text : en4.core.language.translate('Media deleted.')
                    }).inject(smooth.content);
                    smooth.hideLoading();
                    smooth.showWindow();
                    smooth.onLoad();
                    setTimeout(function() {
        ...
        if (this.options.max_files > 0) {
            ...
            if (this.uploader != null ) {
                ...
                // this.hideUploader();
                    ...
                    this.FancyUpload.options.fileListMax = remains_count + this.FancyUpload.fileList.length;
                }
            }
        }
    },
    ...
    addTextBlock: function(block_id) {
        ...
        new Element('button', {
            ...
            tinyMCE.execCommand('mceRemoveControl', false, 'textblock_text_' + gen_block_id);
        ...
        tinyMCE.execCommand('mceAddControl', false, 'textblock_text_' + gen_block_id);
    ...
    doEditTextBlock: function(block_id) {
        ...
        function(response) {
            tinyMCE.execCommand('mceRemoveControl', false, 'textblock_text_' + block_id);
    ...
    editTextBlock: function(block_id) {
        ...
        tinyMCE.execCommand('mceAddControl', false, 'textblock_text_' + block_id);
    },
    isCanAddMedia: function() {
        if (this.options.max_files == 0) return true;
        if ((this.options.max_files - this.options.count_files) > 0) return true;
        return false;
    },
    addVideoServices: function(block_id) {
        if (!this.isCanAddMedia()) return;
        var gen_block_id = Math.floor(Math.random() * (block_id + 1 )*1000) + block_id;
        ...
        new Element('div', {
            'text':en4.core.language.translate("You can add link from other resources. Example:")
        }).inject(block_div);
        ...
        new Element('button', {
            type:"button",
            text:en4.core.language.translate("Get Media")
        }).addEvent('click', function(){
    ...
    addMedia: function(block_id) {
        ...
        new Element('button', {
            type:"button",
            text:en4.core.language.translate("Close")
        }).addEvent('click', function(){
            this.blockCancel(block_div);
        }.bind(this)
            ).inject(block_div);
    ...
    addURL: function(block_id) {
        ...
        new Element('div', {
            'text':en4.core.language.translate("Add a link to the website:")
        }).inject(block_div);
        ...
        new Element('button', {
            type:"button",
            text:en4.core.language.translate("Get Content")
        }).addEvent('click', function(){
...
Smoothbox.Modal.whmediaConfirm = new Class({
    ...
    load : function()
    {
        ...
        new Element('h3', {
            text : en4.core.language.translate('Delete?')
        }).inject(this.content);
        new Element('p', {
            text : en4.core.language.translate('Are you sure that you want to delete it? It will not be recoverable after being deleted.')
        }).inject(this.content);
        var buttons = new Element('div', {
            'class' : 'confirm_buttons'
        });
        new Element('button', {
            type:"button",
            text:en4.core.language.translate("Delete")
...
whmedia.project_likes = new Class({
    ...
    togglelike : function(project_id) {
        this.run_JSON('toggle-like', {
            'id' : project_id
        }, function(response) {                                                                            
...
whmedia.project_follow = new Class({
    ...
    toggle_box : function(a, box_id) {
        this.run_JSON('toggle-box', {
            'box_id': box_id, 
            'user_id' : this.current_follow_user
        }, function(response) {                                                                            
            try {
                if (response.inbox) {
                    a.addClass('a_box');    
    ...
    new_box : function() {
        ...
        Smoothbox.open(null,{
            mode : 'Iframe',
            url: en4.core.baseUrl + 'boxes/create/format/smoothbox/id/' + this.current_follow_user,
            requestMethod : 'get'
        });
    },
    togglefollow : function(user_id) {
        ...
        if (!a_follower_button.hasClass('unfollow')) {
            ...
            try {
                ...
                parent_div.getElement('img.item_photo_user').getParent('a').clone().inject(add2boxes_img).getElement('img.item_photo_user').setStyles({
                    'max-width': '48px'
                });
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            $('profile_photo').getElement('img').clone().inject(add2boxes_img).setStyles({
                                'max-width': '48px'
                            });
                            new Element('span', {
                                'text': $('profile_status').getElement('h2').get('text')
                            }).inject(add2boxes_img);
                        }
                        catch(error1) {}
                    }
                }
            ...
            if (typeOf(div_boxes) == 'element') {
                Smoothbox.open(div_boxes, {
                    mode: 'Inline'
                });
            }
            
        }
        this.run_JSON('toggle-follow', {
            'user_id' : user_id
        }, function(response) {                                                                            
            try {
