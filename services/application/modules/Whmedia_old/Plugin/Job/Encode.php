...
class Whmedia_Plugin_Job_Encode extends Core_Plugin_Job_Abstract
{
  ...
  protected function _process($video)
  {
    ...
    if ($storageObject->mime_major == 'audio') {
        ...
        $outputPath   = $tmpDir . DIRECTORY_SEPARATOR . $video->getIdentity() . '_converted.mp4';
        ...
        if ($settings->getSetting('hd_video_format', 0))
          $hdPath    = $tmpDir . DIRECTORY_SEPARATOR . $video->getIdentity() . '_hdconverted.mp4';
        if ($faad_path !== false) {
            try {
        ...
        else
        {
          ...
          try {
            ...
            $storageObject->save();
            //if ($settings->getSetting('both_video_format', 0)) {
                $secondFileRow = Engine_Api::_()->storage()->create($outputPath, array_merge($params, array('type' => 'video.html5',
                                                                                                            'parent_file_id' => $storageObject->file_id,
                                                                                                            'mime_major' => 'video',
                                                                                                            'mime_minor' => 'mp4'    )));
