<script type="text/javascript">
  en4.core.runonce.add(function(){
    ...
    var anchor = $('profile_whmedia_fprojects').getParent();
    $('profile_whmedia_fprojects_previous').style.display = '<?php echo ( $this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
    $('profile_whmedia_fprojects_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';

    $('profile_whmedia_fprojects_previous').removeEvents('click').addEvent('click', function(){
    ...
    $('profile_whmedia_fprojects_next').removeEvents('click').addEvent('click', function(){
      en4.core.request.send(new Request.HTML({
        url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
        data : {
          format : 'html',
...
<div>
  <div id="profile_whmedia_fprojects_previous" class="paginator_previous">
    <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Previous'), array(
      'onclick' => '',
      'class' => 'buttonlink icon_previous'
    )); ?>
  </div>
  <div id="profile_whmedia_fprojects_next" class="paginator_next">
