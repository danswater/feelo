...
class Formatter
{
    ...
    public static function prettify($css)
    {
        ...
        $cssReplacement = function( $matches ) {
            return esc( base64_encode( $matches[ 0 ] ) );
        };
        ...
        */
        $css = preg_replace_callback( '(url\(data:[^\)]+\))', $cssReplacement, $css );
    	...
    	{
            $oopsReplacement = function( $matches ) {
                return ', ';
            };

    		foreach($m[0] as $oops)
    		{
    			$css = str_replace($oops, preg_replace_callback('#,\r#', $oopsReplacement, $oops), $css);
    		}
    	}
    	
    	$css = preg_replace_callback('#esc\(([^\)]+)\)#', $cssReplacement, $css); // unescape escaped blocks
